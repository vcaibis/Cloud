...
public class RegisterActivity extends AppCompatActivity {
    ...
    public void registerUser(View view) {
        ...
        if(correctUser){
            ...
            com.example.vincent.myapplication.backend.userApi.model.User user = new com.example.vincent.myapplication.backend.userApi.model.User();
            user.setUserName(username);
            user.setPassword(password);
            userMethods.addUser(new User(username,password));
            new EndpointsAsyncTask_User(user).execute();
